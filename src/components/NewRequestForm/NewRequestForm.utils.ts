...
export const featureSchema = z.object({
  ...
  labelGuidelines: z
    .string()
    .min(1, 'Label guidelines are required'),
  exampleLabel: z
    .string()
    .min(1, 'Please provide an example label'),
});
...
export const createOrderSchema = z
  .object({
    ...
    datasetDescription: z
      .string()
      .min(1, 'Dataset description is required'),
    minSamplesCount: z.coerce
      .number()
      .min(1, 'Minimal dataset samples are required'),
    imageGuidelines: z
      .string()
      .min(1, 'Image guidelines are required'),
    exampleImageUrl: z
      .string()
